 ...
 * Gid:    100     100     100     100
 * FDSize: 256         文件描述符的最大个数
 * Groups: 16 33 100   启动该进程用户所属组id
 * VmPeak:     9136 kB 进程地址空间大小
 * VmSize:     7896 kB 虚拟进程空间的大小
 * VmLck:         0 kB 锁住物理内存大小
 * VmPin:         0 kB
 * VmHWM:      7572 kB 内存映射和匿名内存映射大小
 * VmRSS:      6316 kB 正在使用的物理内存大小
 * VmData:     5224 kB 数据段的大小
 * VmStk:        88 kB 用户栈的大小
 * VmExe:       572 kB 程序所拥有的可执行虚拟内存的大小
 * VmLib:      1708 kB 影像内到任务的虚拟内存空间
 * VmPMD:         4 kB
 * VmPTE:        20 kB 所以页表的大小
 * VmSwap:        0 kB
 * Threads:        1   共享该信号描述符的任务的个数
 * SigQ:   0/3067      待处理信号的个数/目前最大可以处理的信号的个数
 * SigPnd: 0000000000000000 屏蔽位，存储了该线程的待处理信号
 * ShdPnd: 0000000000000000 屏蔽位，存储了该线程组的待处理信号
 * SigBlk: 0000000000010000 存放被阻塞的信号
 * SigIgn: 0000000000384004 存放被忽略的信号
 * SigCgt: 000000004b813efb 存放被俘获到的信号
 ...
 * Seccomp:        0
 * Cpus_allowed:   00000001 可以执行该进程的CPU掩码集
 * Cpus_allowed_list:      0
 * Mems_allowed:   1
 * Mems_allowed_list:      0
 * voluntary_ctxt_switches:        150  进程主动切换的次数
 * nonvoluntary_ctxt_switches:     545 进程被动切换的次数
 * </pre>
 *
